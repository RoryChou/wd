...
module.exports =
  class TaskRunner {
    ...
    init () {
      ...
      if (this.isAll) { this.scan(this.basePath) } else { this.scan(this.basePath + '/' + this.arvs[0]) }
    ...
    convertArvsToLowcase () {
      this.arvs.map((currentValue, index) => {
        if (index !== 0 && !currentValue.includes('--')) { return currentValue.toLowerCase() } else { return currentValue }
      }, this.arvs)
    }
    ...
    scan (dirPath) {
      ...
      files.forEach(function (file) {
        ...
        if (stats.isDirectory()) {
          if (that.checkPageDir(fullPath)) { that.taskList.push(fullPath) } else { that.scan(fullPath) }
        }
      })
    }
